...
const PORT = Number(process.env.SKOGCHAT_PORT ?? 9999)
const STATE_DIR = join(homedir(), '.claude', 'channels', 'skogchat')
...
const mcp = new Server(
  { name: 'skogchat', version: '0.1.0' },
  {
    capabilities: { tools: {}, experimental: { 'claude/channel': {} } },
    instructions: `The sender reads the skogchat UI, not this session. Anything you want them to see must go through the reply tool — your transcript output never reaches the UI.\n\nMessages from the skogchat web UI arrive as <channel source="skogchat" chat_id="web" message_id="...">. If the tag has a file_path attribute, Read that file — it is an upload from the UI. Reply with the reply tool. UI is at http://localhost:${PORT}.`,
  },
)
...
mcp.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    {
      name: 'reply',
      description: 'Send a message to the skogchat UI. Pass reply_to for quote-reply, files for attachments.',
      inputSchema: {
        type: 'object',
        properties: {
          text: { type: 'string' },
...
function deliver(id: string, text: string, file?: { path: string; name: string }): void {
  ...
  void mcp.notification({
    ...
    params: {
      ...
      meta: {
        chat_id: 'web', message_id: id, user: 'user', ts: new Date().toISOString(),
...
process.stderr.write(`skogchat: http://localhost:${PORT}\n`)

const HTML = `<!doctype html>
<meta charset="utf-8">
<title>skogchat</title>
<style>
...
function add(m) {
  const who = m.from === 'user' ? 'web' : 'assistant'
