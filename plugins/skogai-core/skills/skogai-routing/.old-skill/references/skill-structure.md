...
<router_pattern_rationale>
<problems_solved>
**Problem 1: Essential context gets skipped.**
If important principles live in a separate reference file, Claude may not read them.
Solution: Put essential principles directly in SKILL.md — they load automatically.

**Problem 2: Wrong references load for the task.**
A "build" task loading debugging references wastes tokens and adds noise.
Solution: Intent detection → specific workflow → workflow declares which references to read.

**Problem 3: Monolithic skills are overwhelming.**
500+ lines of mixed content makes relevant parts hard to find.
Solution: Small router (SKILL.md) + focused workflows + reference library.

**Problem 4: Procedures mixed with knowledge.**
"How to do X" and "What X means" in the same file creates confusion.
Solution: workflows/ = procedures (steps to follow). references/ = knowledge (patterns, context).
</problems_solved>

<when_to_use>
**Router pattern** (SKILL.md + workflows/ + references/) when: multiple distinct workflows, different workflows need different references, essential principles must not be skipped, or skill exceeds ~200 lines.

**Single-file skill** when: one workflow, small reference set, under 200 lines, no principles to enforce.
</when_to_use>
</router_pattern_rationale>
