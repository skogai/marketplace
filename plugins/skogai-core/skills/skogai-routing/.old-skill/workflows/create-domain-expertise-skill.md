...
- ✅ Can be loaded by other routing or planning skills for domain knowledge
...
Explain:
```
Domain expertise skills go in: skills/expertise/{domain-name}/
...
Location: skills/expertise/{suggested-name}/
```
...
**Knowledge reference:** "Can the canonical router or planner load references to plan a project?"
...
## Step 11: Document in the Canonical Router or Planner

Update the repo-local skill that should discover this new domain. If `skogai-routing` is the canonical entrypoint, start there. If a planner skill owns domain inference, update that instead.
...
So the router or planner can auto-detect and offer to load it.
...
- [ ] Can be invoked directly by users
- [ ] Can be loaded by the canonical router or planner for knowledge
...
- [ ] Platform-specific considerations included
- [ ] Located in skills/expertise/{domain-name}/
- [ ] Referenced in the canonical router or planner's domain inference table
