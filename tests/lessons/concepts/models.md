...
1. **Fallbacks** in `agents.defaults.model.fallbacks` (in order).
1. **Provider auth failover** happens inside a provider before moving to the next model.
...
Provider configuration examples (including OpenCode) live in [/providers/opencode](/providers/opencode).
...
- If you omit the provider, OpenClaw resolves the input in this order:
  1. alias match
  1. unique configured-provider match for that exact unprefixed model id
  1. deprecated fallback to the configured default provider If that provider no longer exposes the configured default model, OpenClaw instead falls back to the first configured provider/model to avoid surfacing a stale removed-provider default.
