...
@test "skogai_jq_log writes one compact JSON object per line" {
    bash -c "
        source '$SCRIPTS_DIR/skogai-jq.sh'
    ...
    run bash -c "while IFS= read -r line; do jq -e '.summary and .input.tool_name' >/dev/null <<< \"\$line\" || exit 1; done < /tmp/test-jq-session.jsonl"
    assert_success
}
