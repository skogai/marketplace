...
1. **Chunk IR (format-first)**
   - Chunking happens on the IR text before rendering.
   - Inline formatting does not split across chunks; spans are sliced per chunk.
1. **Render per channel**
...
1. **Render:** implement a renderer with `renderMarkdownWithMarkers(...)` and a style marker map (or Signal style ranges).
1. **Chunk:** call `chunkMarkdownIR(...)` before rendering; render each chunk.
1. **Wire adapter:** update the channel outbound adapter to use the new chunker and renderer.
1. **Test:** add or update format tests and an outbound delivery test if the channel uses chunking.
