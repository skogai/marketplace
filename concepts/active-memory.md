...
This turns the plugin on for the `main` agent, keeps it limited to direct-message style sessions by default, lets it inherit the current session model first, and uses the configured fallback model only if no explicit or inherited model is available.
...
1. enable the plugin
1. target one conversational agent
1. keep logging on only while tuning
...
If you also enable `/trace raw`, the traced `Model Input (User Role)` block will show the hidden Active Memory prefix as:
...
1. **Strict runtime eligibility** Even when enabled and targeted, active memory only runs for eligible interactive persistent chat sessions.
...
`config.modelFallbackPolicy` is retained only as a deprecated compatibility field for older configs. It no longer changes runtime behavior.
...
1. Confirm the current agent id is listed in `config.agents`.
1. Confirm you are testing through an interactive persistent chat session.
1. Turn on `config.logging: true` and watch the gateway logs.
1. Verify memory search itself works with `openclaw memory status --deep`.
...
Active Memory uses the normal `memory_search` pipeline under `agents.defaults.memorySearch`. That means embedding-provider setup is only a requirement when your `memorySearch` setup requires embeddings for the behavior you want.
...
If `memorySearch.provider` is unset, OpenClaw auto-detects the first available embedding provider.
...
Active Memory can still run without embeddings when `memory_search` can operate in degraded lexical-only mode, which typically happens when no embedding provider can be resolved.
