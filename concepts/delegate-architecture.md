...
1. **Scope control**: the identity provider enforces what the delegate can access, independent of OpenClaw's own tool policy.
...
1. **Harden first** — tool restrictions, sandbox, hard blocks, audit trail.
1. **Grant scoped permissions** via the identity provider (least privilege).
1. **Define [standing orders](/automation/standing-orders)** for autonomous operations.
1. **Schedule cron jobs** for recurring tasks.
1. **Review and adjust** the capability tier as trust builds.
