...
1. `parentPeer` match (thread inheritance)
1. `guildId + roles` (Discord role routing)
1. `guildId` (Discord)
1. `teamId` (Slack)
1. `accountId` match for a channel
1. channel-level match (`accountId: "*"`)
1. fallback to default agent (`agents.list[].default`, else first list entry, default: `main`)
