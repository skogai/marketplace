...
1. **Model fallback** to the next model in `agents.defaults.model.fallbacks`.
...
1. Configured `agents.defaults.model.fallbacks` in order.
1. The configured primary model at the end when the run started from an override.

Inside each candidate, OpenClaw tries auth-profile failover before advancing to the next model candidate.

High-level sequence:

1. Resolve the active session model and auth-profile preference.
1. Build the model candidate chain.
1. Try the current provider with auth-profile rotation/cooldown rules.
1. If that provider is exhausted with a failover-worthy error, move to the next model candidate.
1. Persist the selected fallback override before the retry starts so other session readers see the same provider/model the runner is about to use.
1. If the fallback candidate fails, roll back only the fallback-owned session override fields when they still match that failed candidate.
1. If every candidate fails, throw a `FallbackSummaryError` with per-attempt detail and the soonest cooldown expiry when one is known.
...
1. **Configured profiles**: `auth.profiles` filtered by provider.
1. **Stored profiles**: entries in `auth-profiles.json` for the provider.
...
1. Primary fails.
1. Fallback candidate is chosen in memory.
1. Session store still says the old primary.
1. Live-session reconciliation reads the stale session state.
1. The retry gets snapped back to the old model before the fallback attempt starts.
