...
If you want the higher-level protocol context, start with [Gateway architecture](/concepts/architecture).
...
1. Register the method/event in `src/gateway/server-methods-list.ts`.
1. Update `src/gateway/method-scopes.ts` when the new RPC needs operator or node scope classification.
1. Run `pnpm protocol:check`.
1. Commit the regenerated schema + Swift models.
